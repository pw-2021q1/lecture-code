...
/**
 * Save a profile instance in the database
 * Execute different operations depending on the value of the "edit" parameter
 * @param req the request
 * @param res the response
 * @param edit true if the profile is being edited, false if it is being inserted
 */
async function saveProfile(req: e.Request, res: e.Response, edit: boolean) {
    /**
     * Get a field from the multipart request
    ...
    /**
     * Save a profile picture in the "upload" folder
    ...
    /**
     * Parse a new profile from the request
     */
    const parseProfile = function () {
        ...
        return new model.UserProfile(name, birthyear, career, bio)
    }
